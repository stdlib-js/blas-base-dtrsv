...
function dtrsv( uplo, trans, diag, N, A, strideA1, strideA2, offsetA, x, strideX, offsetX ) { // eslint-disable-line max-params, max-len
	...
	if ( u === null ) {
		throw new TypeError( format( '1z5GT', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1z5GG', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1z5GU', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1z5GH', N ) );
	}
	if ( strideA1 === 0 ) {
		throw new RangeError( format( '1z5GB', strideA1 ) );
	}
	if ( strideA2 === 0 ) {
		throw new RangeError( format( '1z5G2', strideA2 ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1z5G3', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
