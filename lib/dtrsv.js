...
function dtrsv( order, uplo, trans, diag, N, A, LDA, x, strideX ) {
	...
	if ( l === null ) {
		throw new TypeError( format( '1z5Fx', order ) );
	}
	u = resolveTriangle( uplo );
	if ( u === null ) {
		throw new TypeError( format( '1z5Fy', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1z5GO', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1z5GP', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1z5GQ', N ) );
	}
	if ( LDA < max( 1, N ) ) {
		throw new RangeError( format( '1z5GR', N, LDA ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1z5GS', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
